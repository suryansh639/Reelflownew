...
import express from "express"; // adjust the path
...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      const enhancedVideos = await Promise.all(
        videos.map(async (video) => {
          const [likeCount, commentCount, userLike] = await Promise.all([
            ...
            user
              ? DynamoDBService.getUserLike(video.id, user.id)
  ...
  app.get("/api/videos/validation-rules", (req, res) => {
    res.json({
      ...
      description:
        "Videos must be educational content only, maximum 60 seconds or 10MB",
    });
  });
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        // Validate file size
        const sizeValidation = VideoValidationService.validateFileSize(
          req.file.size,
        );
        ...
        // Validate file type
        const typeValidation = VideoValidationService.validateMimeType(
          req.file.mimetype,
        );
        ...
        const durationValidation =
          await VideoValidationService.validateVideoDuration(req.file.path);
        ...
        try {
          ...
          return res
            .status(500)
            .json({ message: "Failed to upload video to S3" });
        }
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: "Invalid video data", errors: error.errors });
        }
  ...
  app.post("/api/get-video-url", async (req: any, res) => {
    try {
      ...
      if (!s3Key && !videoUrl) {
        return res
          .status(400)
          .json({ message: "s3Key or videoUrl is required" });
      }
    ...
    try {
      ...
      for (const s3Video of s3Videos) {
        try {
          ...
          const fileName =
            s3Video.key.split("/").pop()?.split(".")[0] || "Untitled";
  ...
  app.post("/api/admin/create-cloudfront", async (req, res) => {
    try {
      ...
      res.json({
        ...
        note:
          "Please set CLOUDFRONT_DOMAIN environment variable to: " +
          distributionDomain,
      });
    } catch (error) {
      ...
      res
        .status(500)
        .json({ message: "Failed to create CloudFront distribution" });
    }
  });
  ...
  app.post("/api/admin/test-cloudfront", async (req, res) => {
    try {
      ...
      const isAccessible = await S3Service.testCloudFrontAccess(
        s3Key || "videos/demo/demo-test.mp4",
      );
      ...
      if (distributionId) {
        try {
          distributionInfo =
            await S3Service.getDistributionInfo(distributionId);
        } catch (error) {
      ...
      res.json({
        ...
        cloudFrontDomain:
          process.env.CLOUDFRONT_DOMAIN || "e2nl5e3zoa6qsv.cloudfront.net",
        testUrl: S3Service.getCloudFrontUrl(
          s3Key || "videos/demo/demo-test.mp4",
        ),
        distributionInfo,
        note: isAccessible
          ? "CloudFront is working!"
          : "CloudFront DNS may still be propagating (takes 5-15 minutes)",
      });
    } catch (error) {
